...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("UBN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("unbrokencoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non UBN unit input in lnReqAmount, we generate the URI with UBN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::UBN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
